...
<script>
    $(document).ready(function() {
        ...
        var table = $('#getHki').DataTable({
            processing: true,
            serverSide: true,
            ajax: "<?= base_url('dosen/getAllhkidetail/' . $nidn) ?>",
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        if (row[1] == <?= session()->get('nidn') ?> || row[3] == <?= session()->get('nidn') ?>) {
                            return '<button class="btn btn-success btn-sm mb-1 mr-1" onclick="edithki(&apos;' + data + '&apos;)">Update</button><button class="btn btn-danger btn-sm mb-1" onclick="deleteshki(&apos;' + data + '&apos;)">Delete</button>';
                        } else {
